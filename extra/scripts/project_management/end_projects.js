...
function asyncquery1 (Model) {
	...
	Model.find({isStarted : true, isOver: false},function(err,res) {
		if(err) {
            throw err;
        }
		...
		res.forEach(function(project,index){
            console.log();
                if(currentdate >= Date.parse(project.End_date)) {
                    console.log(project.isOver);
                    let o = {
...
function asyncquery2 (Model,project) {
	return new Promise(resolve => {
	Model.updateOne( {Name: project.projectname} , { $set: { isOver: true } } , function(err,res) {
		if(err) {
            throw err;
        }
        console.log(res);
    ...
    return new Promise(resolve => {
        ...
        projects.forEach(function(project){
            ...
            project.raspberries.forEach(function(raspberry){
                ...
                let prom = Model2.updateOne( { Device_name : raspberry } , { $set: { Associated_project: "" } } ,function(err,res) {
                    if(err){
                        throw err;
                    }
                    console.log(res);
